...
static void HANDLE_XCB_BUTTON_PRESS() {
  ...
  for each_button {
    /*
     * it would be better with less if clauses, but i think that way is more readable and suggestive
     */

    // ignore if the button hasn't any function or the button pressed isn't the same of the event
    if (!buttons[i].function || buttons[i].button != e->detail) continue;
    // also ignore if the mask is different of the event's
    if (buttons[i].mod_mask != e->state) continue;

    // continue if the action is root only and the event window isn't the root window
    if ((buttons[i].win_mask & ROOT_ONLY) && (e->event != e->root || e->child) ) continue;
...
void (*events[])() = {
#define xmacro(EV) [EV] = HANDLE_##EV,
...
void CLOSE_WINDOW(const union param* param) {
  ...
  // get protocols
  cprop = xcb_icccm_get_wm_protocols_unchecked(conn, focused->id,
                                               ewmh->WM_PROTOCOLS);
...
void MOUSE_MOTION(const union param* param) {
  ...
  if (param->i == MOVE_WINDOW) {
    ...
    /*if you want to implement another possible action, you ca change the cursor
     * here*/
    cursor = dynamic_cursor_get(CURSOR_POINTER);
  }

  cgrab = xcb_grab_pointer(conn, false, root_screen->root,
  ...
  for (; focused;) {
    ...
    switch (gevent->response_type) {
      ...
      case XCB_MOTION_NOTIFY: {
        ...
        if (param->i == MOVE_WINDOW)
          xcb_move_focused_window(wx + mevent->event_x - px,
